# does not work for numerical data
import math
...
def info_gain(data, attribute):
	...
	for attr in unique_attr_values:
		...
		for class_attr_val in unique_class_attr_values:
			temp += p_log_p(count_hash[attr][class_attr_val], count_hash[attr]['total'])
		entropy += (temp * (count_hash[attr]['total']/len(data[class_attr])))
